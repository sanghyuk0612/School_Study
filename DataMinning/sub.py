...
train_ratio = 0.99  # 훈련 세트의 비율을 설정합니다.
...
# # ARIMA 모델 피팅
# ARIMA 모델 생성
model = ARIMA(smoothed_data, order=(1, 2, 2))
...
# model = ARIMA(after_2015, order=(1, 2, 2))  # ARIMA(p, d, q) 에 따라 order 설정
# model_fit = model.fit()

# # 예측하기
# forecast_steps = 100
# forecast = model_fit.forecast(steps=forecast_steps)  # 예측할 기간(steps) 설정
